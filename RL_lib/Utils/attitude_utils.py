...
def make_random_heading_error(theta, los):
    z = np.asarray([0.,0.,1.])
    C = DCM3(los,z)
    theta = np.random.uniform(low=theta[0],high=theta[1])
    phi = np.random.uniform(low=-np.pi,high=np.pi)
    rz = np.cos(theta)
    ry = np.sin(theta)*np.sin(phi)
    rx = np.sin(theta)*np.cos(phi)
    rot_los = np.asarray([rx,ry,rz])
    rot_los = C.T.dot(rot_los)
    return rot_los, theta

def make_random_attitude_error(ap, theta, los, sensor_dvec):
    z = np.asarray([0.,0.,1.])
    C = DCM3(los,z)
    theta = np.random.uniform(low=theta[0],high=theta[1])
    phi = np.random.uniform(low=-np.pi,high=np.pi)
    ...
    rot_los = np.asarray([rx,ry,rz])
    rot_los = C.T.dot(rot_los)
    #z_sensor =  np.asarray([0.,0.,-1.])
    C2 = DCM3(rot_los,sensor_dvec)
    q = ap.dcm2q(C2)
    return  q

def rotate_los(theta, los):
    z = np.asarray([0.,0.,1.])
    C = DCM3(los,z)
    phi = np.random.uniform(low=-np.pi,high=np.pi)
    rz = np.cos(theta)
...
def get_lead(mag_vm, vt, r_tm, debug=False):
    los = r_tm / np.linalg.norm(r_tm)
    unit_vt = vt/np.linalg.norm(vt)
    pn_0 = 1.0*np.asarray([0,0,1])
    
    plane_normal = np.cross(unit_vt,los)
    
    if np.linalg.norm(plane_normal) < 1e-10:  # check for colinear
       
        if debug: 
            print('colinear ', vt, plane_normal)
        vt += 1
        unit_vt = vt/np.linalg.norm(vt)
        plane_normal = np.cross(unit_vt,los)
        if debug: 
            print('fixed ',plane_normal, unit_vt)

    plane_normal = plane_normal / np.linalg.norm(plane_normal)
    C = DCM3(pn_0,plane_normal)
    vt_x = C.T.dot(vt)
    los_x = C.T.dot(los)
    vm = get_lead2d(mag_vm, vt_x, los_x, debug=debug)
    if vm is None:
        return vm
    else:
        vm = C.dot(vm)
...
def get_lead2d(mag_vm, vt, los, debug=False):
    theta_los = np.arctan2(los[1],los[0])
    beta = np.arctan2(vt[1],-vt[0])
    sinL = np.linalg.norm(vt) * np.sin(beta + theta_los) / mag_vm
    if debug:
        print('get_lead2d: ')
        print('\tbeta: ', beta)
        print('\ttheta_los: ', theta_los)
        print('\tsinL: ', sinL)
    if np.abs(sinL) > 1:
        #print('get_lead2d:  Infeasible Intercept (No collision triangle exists)')
        #assert False
        return None
    L = np.arcsin(sinL)
    vm = np.asarray([mag_vm*np.cos(L+theta_los),mag_vm*np.sin(L+theta_los),0.0])
    return(vm)
