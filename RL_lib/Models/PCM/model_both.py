...
class Model(object):
    ...
    def fit1(self, rollouts, logger):
        ...
        if self.test_pred:
            self.test_lt(unscaled_obs, unscaled_act,  unscaled_nobs, states, errors, masks, flags, unscaled_nobs, sdr,  self.test_steps)
    ...
    def predict(self, unscaled_obs, unscaled_act,  unscaled_nobs, states, errors, masks, flags, scale_outputs=True):
        ...
        pred, dpred = np.split(pred , 2, axis=1)
        targ = nobs
        next_error1 = pred - nobs
        next_error2 = dpred - delta_actual 
        if scale_outputs:
            delta = self.delta_scaler.reverse(pred)
            pred = unscaled_obs + delta
        else:
    ...
    def test_lt(self, unscaled_obs, unscaled_act,  unscaled_nobs, states, errors, masks , flags, targ, sdr,  steps):
        ...
        for t in range(steps):
            ...
            pred, vpred, s, _ = self.predict(pred, unscaled_act[t],  unscaled_nobs[t], s, e, masks[t], flags[t], scale_outputs=True)
