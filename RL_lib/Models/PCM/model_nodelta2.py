...
class Model(object):
    ...
    def __init__(self, net, epochs=20,  shuffle=False, max_grad_norm=999, p_coeff=0.85 , cliprange=0.5, test_pred=False, 
                 ...
                 model_idx=None, learn_rewards=False):
        print('PCM Model1:')
        net.apply(init_func)
        self.net = net
    ...
    def fit1(self, rollouts, logger):
        ...
        targets = self.nobs_scaler.apply(unscaled_nobs)
    ...
    def get_initial_predict(self, unscaled_obs, scale_outputs=True):
        ...
        if scale_outputs:
            pred = self.nobs_scaler.reverse(pred)
        return pred,vpred 
    ...
    def test_lt(self, unscaled_obs, unscaled_act,  unscaled_nobs, states, errors, masks , flags, targ, sdr,  steps):
        ...
        s = states[0]
        pred = unscaled_obs[0]
        flags_inv = np.logical_not(flags)
        masks = 1.0*np.logical_and(flags_inv, masks)
