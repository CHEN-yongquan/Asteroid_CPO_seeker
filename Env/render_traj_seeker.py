...
def render_traj(traj, target_position=None):
    ...
    plt.plot(t1,pc[:,0],'r',label=r'$\theta_u$' )
    plt.plot(t1,pc[:,1],'b',label=r'$\theta_v$' )
    plt.legend(bbox_to_anchor=(0., 0.98, 1., .102), loc=3,
            ncol=5, mode="expand", borderaxespad=0.)
    plt.gca().set_ylabel('Seeker Angles (rad)')
    plt.gca().set_xlabel("Time (s)")
    plt.grid(True)

    plt.subplot2grid( (4,2) , (1,0) ) 
    opt_flow=np.asarray(traj['optical_flow'])[0:-1]
    plt.plot(t1,np.asarray(opt_flow[:,0]),'r',label=r'$d\theta_u/dt$')
    plt.plot(t1,np.asarray(opt_flow[:,1]),'b',label=r'$d\theta_v/dt$')
    plt.legend(bbox_to_anchor=(0., 1.00, 1., .102), loc=3,
            ncol=5, mode="expand", borderaxespad=0.)
    plt.gca().set_xlabel("time (s)")
    plt.gca().set_ylabel('Seeker Angle ROC (rad/s)')
    plt.grid(True)

    plt.subplot2grid( (4,2) , (1,1) )
    tcv = np.asarray(traj['theta_cv'])
    plt.plot(t,tcv,'r',label='Theta_BV')
    plt.legend(bbox_to_anchor=(0., 1.00, 1., .102), loc=3,
            ncol=5, mode="expand", borderaxespad=0.)
    plt.gca().set_xlabel("Time (s)")
    plt.gca().set_ylabel('Theta BV (rad)')
    plt.grid(True)
