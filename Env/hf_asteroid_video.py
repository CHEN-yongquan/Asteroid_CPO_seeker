...
class Asteroid(object):
    ...
    def generate_random_surface_position(self):
        x = self.a_axis * np.cos(self.p_theta) * np.cos(self.p_phi)
        y = self.b_axis * np.cos(self.p_theta) * np.sin(self.p_phi)
        z = self.c_axis * np.sin(self.p_theta)
