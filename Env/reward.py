...
class Reward(object):
    ...
    def __init__(self,  reward_scale=1.0, ts_coeff=-0.0,  fuel_coeff=-0.01, landing_coeff = 10.0, 
                 ...
                 optflow_coeff=0.0, fov_coeff=0.0, fov_alt=5.0, offset=np.zeros(2) ):
    ...
    def get(self, lander,  action, done, steps,  glideslope_constraint, attitude_constraint, w_constraint, rh_constraint):
        ...
        fov_penalty = 0.0
        rh_penalty = 0.0
        pixel_coords = (lander.sensor.centered_pixel_coords - self.offset) / (lander.sensor.camera_model.p_x // 2)
        ...
        if alt > 0 and not lander.sensor.check_for_vio():
            ...
            r_tracking = self.tracking_bias + magv_error + tracking_error  
        else:
            r_tracking = 0.0
        ...
        if done:
            ...
            if np.linalg.norm(pos) < self.landing_rlimit and np.linalg.norm(vel) < self.landing_vlimit: 
