...
class Isosphere(object):
    ...
    def show_traj(self, traj, axis_limit=1, marker_loc=[500.,500.,500.], offset=np.zeros(3)):
        marker_loc=traj[-1]
        V = self.faces2vertices() + offset
        ax = a3.Axes3D(pl.figure())
        for vtx in V:
            tri = a3.art3d.Poly3DCollection([vtx])
            tri.set_color('tab:gray')
            tri.set_edgecolor('k')
        ...
        ax.scatter(traj[:,0], traj[:,1], traj[:,2],s=0.1,label='Lander Trajectory')
        ax.scatter(marker_loc[0], marker_loc[1], marker_loc[2],  marker='o', color='m',label='Tagged LZ')
        ax.scatter(traj[0,0], traj[0,1], traj[0,2], marker='o', color='k',label='Hovering Spacecraft')
        beam_x = [traj[0,0] , traj[-1,0]]
        beam_y = [traj[0,1] , traj[-1,1]]
        beam_z = [traj[0,2] , traj[-1,2]]
        line = a3.art3d.Line3D(beam_x, beam_y, beam_z, lw=0.5, color='m',label='Targeting Beam')
        line.set_zorder(20)
        print(tri.get_zorder(), line.get_zorder())
        ax.add_line(line)
        #ax.annotate('annotate', xy=(2, 1), xytext=(300, 400),
        #    arrowprops=dict(facecolor='black', shrink=0.05))
        #ax.text(0, 0, 500, r'an equation: $E=mc^2$', fontsize=15)
        pl.gca().set_zlabel('Z (m)')
        pl.gca().set_ylabel('Y (m)')
        pl.gca().set_xlabel("X (m)")
        lgnd = pl.legend(bbox_to_anchor=(0., 0.90, 1., .102), loc=3,
    ...
    def scale(self, scale_factor):
        p_vertices = np.vstack(self.vertices)
        p_vertices *= scale_factor
        self.vertices = list(p_vertices)
